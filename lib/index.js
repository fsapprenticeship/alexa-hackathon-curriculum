...
// Learning Opportunity: A sample Alexa Skill Lambda function
...
const YesHandler = {
    ...
    handle(handlerInput) {
        ...
        const speechOutput = `Here the list of skills you can learn ${getListSkill()}. Which skill do you want to learn more?`
        return responseBuilder
            .speak(speechOutput)
            .getResponse();
    },
...
const NoHandler = {
    ...
    handle(handlerInput) {
        ...
        const speechOutput = `Which skill do you want to learn more?`
        return responseBuilder
            .speak(speechOutput)
            .getResponse();
    },
...
const SkillHandler = {
    canHandle(handlerInput) {
        ...
        return request.type === 'IntentRequest' 
            && (request.intent.name === 'AlgorithmIntent'
            || request.intent.name === 'TypescriptIntent'
            || request.intent.name === 'JavaIntent');
    },
    handle(handlerInput) {
        const request = handlerInput.requestEnvelope.request;
        const responseBuilder = handlerInput.responseBuilder;
        // let intent = getLanguageIntent(request.intent.name);
        // const speechOutput = `Some topics you should cover to study ${intent} ${learningData.courses.find(e => e.name === itent).skills.join(' ')}`;

        return responseBuilder.speak("test here").getResponse();
    },
};
const GoOutHandler = {
    canHandle(handlerInput) {
...
const learningData = {
    courses: [{
        id: 1,
        name: 'Algorithm',
        skills: ['Dynamic Programming', "Recursive","Hash Table", "LinkedList"]
    },
    {
        id: 2,
        name: 'Typecript',
        skills: ["Type", "Promise", "Worker"]
    }, 
    {
        id: 3,
        name: 'Java',
        skills: ["JUnit", "Array", "Spring Framework"]
    }]
}
const languageStrings = {
    en: {
        translation: {
            WELCOME: 'Welcome to Learning Opportunity',
            HELP: `We have several learning opportunity. Do you want to hear the list? `,
...
function getListSkill() {    
    return learningData.courses.reduce((prev, current) => {
...
function getLanguageIntent(intent) {
    switch(intent) {
        case 'AlgorithmIntent': return 'algorithm';
        case 'TypescriptIntent': return 'typescript';
        case 'JavaIntent': return 'java';
        default: return "";
    }
}
